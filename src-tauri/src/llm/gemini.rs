...
pub struct ParsedChunk {
    ...
    /// Tool calls. Gemini delivers these whole rather than in fragments.
...
/// A tool call, complete in one part.
...
struct WireFunctionCall {
    #[serde(default)]
    name: String,
    /// Absent for a tool that takes no arguments.
    #[serde(default)]
    ...
    for candidate in chunk.candidates {
        if let Some(content) = candidate.content {
            for part in content.parts {
                // A functionCall part carries no text at all, so it has to be
                // read before the text guard rather than after it.
