...
export interface PackAnimation {
  ...
  offset: Offset;
  /**
   * The frame's own pixel dimensions, read from the file by Rust.
   *
   * Frames in a pack are not all the same size, and the offsets beside them
   * assume a frame drawn at its natural size. Fitting each one to a shared box
   * would give every animation its own scale factor.
   */
  size: FrameSize;
...
export type AgentState =
  ...
  | "confused"
  /**
   * Dozing after a long stretch of no interaction.
   *
   * A pack may map its own animation; none is required, and the fallback to
   * `idle` plus a dimmed treatment reads as asleep well enough on a pack that
   * has no sleeping frames. Waking is any interaction at all.
   */
  | "sleep";
