...
export function Sprite({ pack, geometry, onActivate, onContextMenu }: SpriteProps) {
  ...
  const asleep = useAmbientStore((s) => s.asleep);

  const animation = resolveAnimation(pack, agentState, emoteOverride);

  // The shared box: big enough for the largest frame in the pack.
  const width = pack.frameSize.width * scale;
  const height = pack.frameSize.height * scale;

  /*
   * Each frame is drawn at its *own* size, centred in that box, then nudged by
   * its registration offset — which is what the offsets in the pack mean. The
   * frames here run from 257x278 to 302x298, so stretching each to fill the box
   * would scale them by different amounts and make her change size every time
   * the state changed, which is the exact jump the offsets exist to remove.
   */
  const frame = animation?.size ?? pack.frameSize;
  const frameWidth = frame.width * scale;
  const frameHeight = frame.height * scale;
  const offsetX = (animation?.offset.x ?? 0) * scale;
  const offsetY = (animation?.offset.y ?? 0) * scale;
  ...
  live.current = {
    ...
    geometry,
    pinned,
    // The drawn frame, which is what the hit mask has to describe — not the box
    // it is centred in.
    frameWidth,
    frameHeight,
    offsetX,
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      // Centre of the box, then the frame's own box around that centre.
      const { frameWidth: fw, frameHeight: fh, offsetX: ox, offsetY: oy } = live.current;
      publishMask(
        left + (w - fw) / 2 + ox,
        top + (h - fh) / 2 + oy,
        fw,
  ...
  return (
    <div
      ref={containerRef}
      className={`sprite${asleep ? " sprite--asleep" : ""}`}
      style={{ width, height }}
